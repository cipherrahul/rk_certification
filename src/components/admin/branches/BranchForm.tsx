...
import { createBranchAction, updateBranchAction } from "@/lib/actions/branch.action";
...
export function BranchForm({ initialData, isEdit }: BranchFormProps) {
    ...
    const form = useForm<BranchFormValues>({
        ...
        defaultValues: initialData ? {
            ...
            openingDate: new Date(initialData.opening_date),
    ...
    async function onSubmit(values: BranchFormValues) {
        ...
        if (isEdit && initialData?.id) {
            res = await updateBranchAction(initialData.id, values);
        } else {
            res = await createBranchAction(values);
        }
        ...
        if (res.success) {
            ...
            toast({
                variant: "destructive",
                title: "Error",
                description: res.error || `Failed to ${isEdit ? "update" : "create"} branch.`,
            });
        }
        setLoading(false);
    }
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="flex justify-end gap-4">
                    ...
                    <Button type="submit" className="bg-brand hover:bg-brand/90 text-white min-w-[150px]" disabled={loading}>
                        {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4 mr-2" /> {isEdit ? "Update Branch" : "Save Branch"}</>}
                    </Button>
                </div>
            </form>
        </Form>
