...
export const generateBrochure = async (program: Program) => {
    ...
    // Large Category Text (Watermark style)
    doc.setTextColor(white);
    doc.setFontSize(60);
    doc.setFont('helvetica', 'bold');
    doc.text(program.category.substring(0, 4).toUpperCase(), pageWidth - 30, 240, { angle: 90, charSpace: 5, opacity: 0.1 });
    ...
    currentY += 15;
    doc.setFontSize(44);
    const titleLines = doc.splitTextToSize(program.title, pageWidth - (margin * 2) - 20);
    ...
    doc.text(`Comprehensive guide to ${program.category} excellence`, margin, currentY);

    // Contact & Location Card (Bottom)
    ...
    program.fullCurriculum.forEach((module, idx) => {
        if (currentY > pageHeight - 50) {
            doc.addPage();
            addPageDecoration(2); // Simplified
            currentY = 30;
        }
    ...
    const leadership = [
        {
            ...
            bio: "M.Sc Mathematics with a mission to simplify competitive learning. Over a decade of experience in mentoring students for JEE, CUET, and Foundation levels.",
            img: "/images/faculty/manish.jpg"
        },
        {
            name: "Rahul Choudhary",
            role: "Co-founder & Software Engineer",
            bio: "B.Tech Computer Science leading technical innovation. Expertise in Full Stack Engineering and Modern EdTech systems to bridge the gap between education and industry.",
    ...
    // Outcomes Section (Horizontal Modern)
    doc.setTextColor(deepSlate);
    doc.setFontSize(18);
    doc.setFont('helvetica', 'bold');
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    const footerLines = doc.splitTextToSize('Join a community of thousands of successful students who have transformed their careers with RK Institution. Our admissions experts are ready to guide you.', pageWidth - (margin * 2));
    ...
    doc.save(`${program.slug}-brochure-official.pdf`);
};
